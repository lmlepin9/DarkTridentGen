...
int main(int argc, char** argv)
{
  ...
  //Get user inputs
  char c;
  while((c = getopt(argc, argv, "i:x:c:o:s:h:m:r:")) != -1) {
    switch(c) {
      case 'i':
        infn = optarg;
        break;
  ...
  TTree *intree = (TTree *) fin->Get("event_tree");
  ...
  intree->SetBranchAddress("dm_vx",&vx);
  intree->SetBranchAddress("dm_vy",&vy);
  intree->SetBranchAddress("dm_vz",&vz);
  intree->SetBranchAddress("dm_vt",&vt);
  intree->SetBranchAddress("channel_name", &name);
  //intree->SetBranchAddress("id",&origin_id);
  double maxE = intree->GetMaximum("dm1_energy");
  //const double maxL = inttree->GetMaximum("L");

  TTree *pot_tree = (TTree *)fin->Get("pot_tree");
  ...
  if(!outf.empty()) {
    ...
    ot->Branch("pot_per_event", &pot_per_event);
    cout << outf << endl;
  }
