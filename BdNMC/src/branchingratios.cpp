...
double dbrpi0_to_gamma_dm_dm(double mv, double mx, double kappa, double alphaD, double s, std::string dm_type){
    ...
    /*
    //Setup for extra term in matrix element 
    ...
    double mass[Nop] = {mx,mx,mgamma};
    pb[0].SetPxPyPzE( 0.0, 0.0, 0., mpi0);
    _decay->SetDecay(pb[0], Nop, mass);
    double wtdecay =  2*_decay->Generate();
    //get out particles
    ...
    double dot_chi1 = pf[3].Dot(pf[1]);
    double dot_chi2 = pf[3].Dot(pf[2]);

    //std::cout << "Dot chi1 " << dot_chi1 << std::endl;
    //std::cout << "Dot chi2 " << dot_chi2 << std::endl;
...
double d2breta_to_gamma_dm_dm(double mv, double mx, double kappa, double alphaD, double s, double theta, std::string dm_type){
    ...
    if(dm_type == "scalar"){
        ...
        d2breta = bretato2gamma/brpi0to2gamma*sqrt(1-4.0*mx*mx/s)*pow(pow(meta,2)-s,3)*(s+2*mx*mx)*4*alphaD*pow(kappa,2)*pow(sin(theta),3)/(8*pow(meta,6)*pi*(pow(s-mv*mv,2)+pow(mv*GammaV(mv,mx,kappa,alphaD),2)));
...
double dbreta_to_gamma_dm_dm(double mv, double mx, double kappa, double alphaD, double s, std::string dm_type){
    ...
    TLorentzVector pf[Nop+1];
    double mass[Nop] = {mx,mx,mgamma};
    pb[0].SetPxPyPzE( 0.0, 0.0, 0., meta);
    _decay->SetDecay(pb[0], Nop, mass);
    double wtdecay =  2*_decay->Generate();
    //get out particles
	for( int i = 1; i < Nop+1; i++ )
    ...
    */ 
    ...
    if(dm_type == "scalar"){
        ...
        //dbreta = bretato2gamma/brpi0to2gamma*sqrt(1.0-4.0*mx*mx/s)*(pow(meta,2)-s)*(pow(pow(meta,2)-s,2)*(s+2.0*mx*mx) -8*s*dot_chi1*dot_chi2)*4*alphaD*pow(kappa,2)/(6.0*pow(meta,6)*pi*(pow(s-mv*mv,2)+pow(mv*GammaV(mv,mx,kappa,alphaD),2)));
    }
    return dbreta;
}
