...
bool pion_decay_gen::GenDM(std::list<Particle>& vec, std::function<double(Particle&)> det_int, Particle& part){
    ...
    /*
        This function returns true if the decay of a pion into a pair of DM particles is successful.
        The simulated decay is only considered if at least one of the DM particle intersects the detector. 
        The mother and daughter particles of the decay are pushed to an empty list of Particles. 

        Arguments:
            - std::list<Particle> &vec : An empty list of Particles where the parent meson and daughters
                                        will be pushed
            
            - std::function<double(Particle&)> det_int : A function that can be used to check if a DM particle
            intersects the detector

            - Particle& part: Reference to an input meson sampled from a particle distribution (could be Sadnford-Wang) or your 
                            own list of pions. 

        Returns:
            bool : true if the decay is successfully simulated. 
    */
