  ...
  for (list<production_channel>::iterator proditer = prodlist->begin(); proditer != prodlist->end(); proditer++)
  {
    ...
    else if (proddist == "parton_V" || proddist == "parton_V_baryonic")
    {
      ...
      std::shared_ptr<parton_sample> parsam(new parton_sample(proton_file, neutron_file,
                                                              target_p, target_n));
      Vnum = POT * parsam->production_proton_cross_section() * microbarn / target_p_cross * (target_p + target_n);
    ...
    // For axion dark photon model, and meson decay. Not actually used in dark tridents
    if (par->Model_Name() == "Axion_Dark_Photon" || (par->Model_Name() == "Dark_Photon" && sigchoice == "Signal_Decay"))
    {
      ...
      else if (prodchoice == "pi0_decay" || prodchoice == "eta_decay")
      {
        ...
        if (prodchoice == "pi0_decay")
        {
          ...
          DMGen = std::shared_ptr<DMGenerator>(new Two_Body_Decay_Gen(brpi0toVgamma(mv, mdm, kappa, alD),
        ...
        else if (prodchoice == "eta_decay")
        {
          ...
          DMGen = std::shared_ptr<DMGenerator>(new Two_Body_Decay_Gen(bretatoVgamma(mv, mdm, kappa, alD),
    ...
    else if (prodchoice == "pi0_decay" || prodchoice == "pi0_decay_baryonic")
    {
      ...
      if (proditer->Meson_Per_Pi0() <= 0 && proddist != "particle_list")
      {
        Vnum = DMGen->BranchingRatio() * num_pi0;
      }
      else if (proditer->Meson_Per_Pi0() <= 0 && proddist == "particle_list")
      {
        std::cout << "Using a meson list distribution, the number of pions is taken from the input file and POT normalized..." << std::endl;
        Vnum = POT * DMGen->BranchingRatio() * numLines / external_pot;
      }
