...
bool Nucleon_Scatter::probscatter(std::shared_ptr<detector>& det, Particle &DM){ 
    /*
        This function is used to calculate the probability of scatter during
        the Burn-in loop. 

        Arguments:
        * Detector geometry
        * A incoming DM particle 
        

        Returns:
        True if probability of scattering is greater than zero 
    ...
    */
    ...
    double prob=LXDet*convGeV2cm2*XDM*DM.w;
    //cout << prob << endl;
...
bool Nucleon_Scatter::probscatter(std::shared_ptr<detector>& det, list<Particle>& partlist, list<Particle>::iterator& DMit){
    ...
    /*
    Returnts true if the probability of DM-nucleon scattering is greater than zero
    and if angle cuts apply.
    It also simulates the recoil DM and nucleus.
    First it generates a position and links it with the nucleon
    Then it inserts both to the particle list 
    */
...
bool Nucleon_Scatter::probscatter(std::shared_ptr<detector>& det, Particle &DM, Particle &Nucleon){ 
    /*
      Function that simulates the DM-nucleon scattering

      Arguments:
      - detector object 
      - Particle DM: Incoming DM 
      - Particle Nucleon: Target nucleon 

      Returns True if the prob of scatter is greater than zero 


      LM Comment: I need to add the DM weights here! 
    */
	using namespace std::placeholders;
    double LXDet = convmcm*(det->Ldet(DM));
    if(coherent){    
        ...
        double prob=LXDet*convGeV2cm2*(XDMp+XDMn)*DM.w;
        if(prob > pMax*Random::Flat(0,1)){
            if(prob > pMax)
                pMax = prob;
